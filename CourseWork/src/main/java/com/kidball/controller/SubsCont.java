...
// загружает страницу для добавления нового абонемента.
    @GetMapping("/add")
    public String SubAdd(Model model) {
        AddAttributes(model);
        return "subAdd";
    }
//загружает страницу для редактирования существующего абонемента на основе переданного идентификатора (id).
    @GetMapping("/edit/{id}")
    public String SubEdit(Model model, @PathVariable Long id) {
        AddAttributesSubEdit(model, id);
        return "subEdit";
    }
//удаляет абонемент с указанным идентификатором (id).
    @GetMapping("/delete/{id}")
    public String SubDelete(@PathVariable Long id) {
        subsRepo.deleteById(id);
        return "redirect:/subs";
    }
//обрабатывает отправку формы добавления нового абонемента. Он сохраняет данные абонемента и загружает соответствующее изображение (если предоставлено) на сервер.
    @PostMapping("/add")
...
// обрабатывает отправку формы редактирования существующего абонемента. Он обновляет данные абонемента, включая изображение (если предоставлено).
    @PostMapping("/edit/{id}")
